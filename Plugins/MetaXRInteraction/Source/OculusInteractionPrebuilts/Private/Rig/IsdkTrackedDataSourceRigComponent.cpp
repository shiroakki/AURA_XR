...
{
  const auto TrackingData = GetTrackingDataSubsystem();

  LocalDataSources = {};
  if (IsValid(TrackingData.GetObject()))
  {
    LocalDataSources = IIsdkITrackingDataSubsystem::Execute_CreateHandDataSourceComponent(
        TrackingData.GetObject(), AttachedToMotionController);
  }
...
void UIsdkTrackedDataSourceRigComponent::CreateDataSourcesAsTrackedController()
{
  ...
  if (IsValid(TrackingData.GetObject()))
  {
    LocalDataSources = IIsdkITrackingDataSubsystem::Execute_CreateControllerDataSourceComponent(
        TrackingData.GetObject(), AttachedToMotionController);
  }
...
void UIsdkTrackedDataSourceRigComponent::UpdateComponentVisibility()
{
  ...
  if (!IsValid(TrackedVisual))
  {
    TrackedVisual = nullptr;
  }
  auto* SyntheticVisual = GetSyntheticVisual();
  if (!IsValid(SyntheticVisual))
  {
    SyntheticVisual = nullptr;
  }

  if (IIsdkIRootPose::Execute_IsRootPoseConnected(HandRootPoseObj)->GetResolvedValue())
  {
    if (HandVisibilityMode == EIsdkRigHandVisibility::SyntheticOnly && SyntheticVisual != nullptr)
    {
