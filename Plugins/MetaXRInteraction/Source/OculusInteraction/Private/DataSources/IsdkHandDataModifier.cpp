...
void UIsdkHandDataModifier::SetRecursiveUpdate(bool InRecursiveUpdate)
{
  RecursiveUpdate = InRecursiveUpdate;
  if (IsApiInstanceValid())
  {
    GetApiIHandDataModifier()->setRecursiveUpdate(InRecursiveUpdate ? 1 : 0);
  }
}
...
UIsdkConditional* UIsdkHandDataModifier::IsRootPoseConnected_Implementation()
{
  check(IsValid(InputDataSource));
  return Execute_IsRootPoseConnected(InputDataSource);
}

UIsdkConditional* UIsdkHandDataModifier::IsRootPoseHighConfidence_Implementation()
{
  check(IsValid(InputDataSource));
  return Execute_IsRootPoseHighConfidence(InputDataSource);
}
