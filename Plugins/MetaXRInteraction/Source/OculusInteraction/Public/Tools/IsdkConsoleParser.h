 ...
 public:
  ...
  static void ParseConsoleCommand(const TArray<FString>& TextArgs, UWorld* World)
  {
    if (!TextArgs.IsEmpty())
    {
      ...
      if (TextArgs[0] == "showdebug")
      {
        // add any other debug-enabling calls here
        EnableDebugRenderingForGrabbing(World, true);
      }
    }
  }

  static void EnableDebugRenderingForGrabbing(UWorld* world, bool show)
  {
    TArray<AActor*> actors;
    UGameplayStatics::GetAllActorsOfClass(world, AActor::StaticClass(), actors);
    for (AActor* actor : actors)
    {
      UIsdkGrabbableComponent* grabbable = actor->GetComponentByClass<UIsdkGrabbableComponent>();
      if (IsValid(grabbable))
      {
        grabbable->SetDebugRendering(show);
      }
      UIsdkGrabberComponent* grabber = actor->GetComponentByClass<UIsdkGrabberComponent>();
      if (IsValid(grabber))
      {
        grabber->SetDebugRendering(show);
      }
    }
  }
};
